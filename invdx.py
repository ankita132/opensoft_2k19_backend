...
#create and store inverted index using this code
#the key values refer to all the vocabulary in the dataset
#the values for each key(word in vocabulary) is a dictionary mapping the documents and the term frequency in each document 
class InvertedIndex:
...
#create a document length table wth the document names as keys and the document length as their value.
...
def build_data_structures(corpus):
	...
	#storing inv index and doc_length_table as pickle files
	with open("../inv_index.pkl",'wb') as f:
		pickle.dump(idx,f)

	with open("../doc_length.pkl",'wb') as f2:
		pickle.dump(dlt,f2)
